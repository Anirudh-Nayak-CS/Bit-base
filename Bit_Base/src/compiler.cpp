#include <bits/stdc++.h>
#include "../headers/compiler.h"
#include "../headers/storage/database/db.h"
using namespace std;

SQLcommandType getCommandType(const std::string &cmd)
{
  if (cmd == "INSERT")
    return SQLcommandType::INSERT;
  if (cmd == "SELECT")
    return SQLcommandType::SELECT;
  if (cmd == "DELETE")
    return SQLcommandType::DELETE;
  if (cmd == "UPDATE")
    return SQLcommandType::UPDATE;
  if (cmd == "CREATE TABLE")
    return SQLcommandType::CREATE_TABLE;
  if (cmd == "DROP TABLE")
    return SQLcommandType::DROP_TABLE;
  return SQLcommandType::UNDEFINED;
}

bool handle_meta_commands(string meta_command)
{
  if (meta_command == ".exit")
  {
    cout << "EXITED" << endl;
    exit(EXIT_SUCCESS);
  }
  else if (meta_command == ".tables")
  {
    cout << "Users" << endl;
    return true;
  }
  else if (meta_command == ".help")
  {
    cout << "--- Meta Commands ---" << endl;
    cout << "  .exit      : Exit the database REPL" << endl;
    cout << "  .help      : Print this message" << endl;
    cout << "  .tables    : List all tables" << endl;

    cout << "\n--- SQL Commands ---" << endl;
    cout << "  insert <id> <username> <email> <age> <gender> : Add a new row" << endl;
    cout << "  select                                        : Print all rows" << endl;
    cout << "  update <id> <username> <email> <age> <gender> : Modify an existing row" << endl;
    cout << "  delete <id>                                   : Remove a row" << endl;
    cout << "  create table <name>                           : Create a new table" << endl;
    cout << "  drop table <name>                             : Delete a table" << endl;

    return true;
  }
  else
  {
    cout << "Unrecognized command .cmd" << endl;
    return false;
  }
}

void get_current_time(char *buff, size_t size)
{
  time_t time_now = time(NULL);
  tm *t = localtime(&time_now);
  strftime(buff, size, "%Y-%m-%d %H:%M:%S", t);
}

Commandstatus handle_insert(string sql_command, stmt &statement, db *database)
{
  stringstream ss(sql_command);
  string first_word, second_word;

  if (!(ss >> first_word))
    return Commandstatus::CMD_SYNTAX_ERROR;

  if (!(ss >> second_word) || second_word != "INTO")
    return Commandstatus::CMD_SYNTAX_ERROR;

  string table_name;
  if (!(ss >> table_name))
    return CMD_SYNTAX_ERROR;
  
  Table *table = database->getTable(table_name);

  int id;
  string username;
  string email;

  // id
  if (!(ss >> id))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (id < 0)
  {

    return Commandstatus::CMD_NEGATIVE_INT;
  }

  // username
  if (!(ss >> username))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (username.length() > 32)
  {

    return Commandstatus::CMD_STRING_TOO_LONG;
  }

  // email
  if (!(ss >> email))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (email.length() > 255)
  {

    return Commandstatus::CMD_STRING_TOO_LONG;
  }

  // age
  int age;
  if (!(ss >> age))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (age < 0 || age > 100)
  {

    return Commandstatus::CMD_OUT_OF_RANGE;
  }

  // gender
  gender sex;
  string gen;

  if (!(ss >> gen))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (gen == "male" || gen == "MALE" || gen == "female" || gen == "FEMALE" || gen == "OTHER" || gen == "other")
  {
    if (gen == "male")
    {
      sex = MALE;
    }
    else if (gen == "female")
    {
      sex = FEMALE;
    }
    else
    {
      sex = OTHER;
    }
  }
  else
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  string leftover;
  if (ss >> leftover)
    return Commandstatus::CMD_SYNTAX_ERROR;

  uint32_t final_id = static_cast<uint32_t>(id);
  char final_username[32];
  char final_email[255];

  strncpy(final_username, username.c_str(), sizeof(final_username));
  strncpy(final_email, email.c_str(), sizeof(final_email));
  final_username[sizeof(final_username) - 1] = '\0';
  final_email[sizeof(final_email) - 1] = '\0';

  statement.row_data.id = final_id;
  strncpy(statement.row_data.username, final_username, sizeof(statement.row_data.username) - 1);
  strncpy(statement.row_data.email, final_email, sizeof(statement.row_data.email) - 1);

  statement.id = final_id;
  statement.type = INSERT;

  cout << " insertion parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}
Commandstatus handle_update(string sql_command, stmt &statement)
{
  stringstream ss(sql_command);
  string words;
  if (!(ss >> words))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  int id;
  string username;
  string email;

  // id
  if (!(ss >> id))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (id < 0)
  {

    return Commandstatus::CMD_NEGATIVE_INT;
  }

  // username
  if (!(ss >> username))
  {

    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (username.length() > 32)
  {

    return Commandstatus::CMD_STRING_TOO_LONG;
  }

  // email
  if (!(ss >> email))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (email.length() > 255)
  {

    return Commandstatus::CMD_STRING_TOO_LONG;
  }

  // age
  int age;
  if (!(ss >> age))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (age < 0 || age > 100)
  {

    return Commandstatus::CMD_OUT_OF_RANGE;
  }

  // gender
  gender sex;
  string gen;

  if (!(ss >> gen))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (gen == "male" || gen == "female" || gen == "other")
  {
    if (gen == "male")
    {
      sex = MALE;
    }
    else if (gen == "female")
    {
      sex = FEMALE;
    }
    else
    {
      sex = OTHER;
    }
  }
  else
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  string leftover;
  if (ss >> leftover)
    return Commandstatus::CMD_SYNTAX_ERROR;

  uint32_t final_id = static_cast<uint32_t>(id);
  char final_username[32];
  char final_email[255];

  strncpy(final_username, username.c_str(), sizeof(final_username));
  strncpy(final_email, email.c_str(), sizeof(final_email));
  final_username[sizeof(final_username) - 1] = '\0';
  final_email[sizeof(final_email) - 1] = '\0';

  statement.id = final_id;
  statement.type = UPDATE;

  cout << "updation parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}

Commandstatus handle_delete(string sql_command, stmt &statement)
{
  stringstream ss(sql_command);
  string words;

  if (!(ss >> words))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  int id;

  // id
  if (!(ss >> id))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  if (id < 0)
  {
    return Commandstatus::CMD_NEGATIVE_INT;
  }

  string leftover;
  if (ss >> leftover)
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  uint32_t final_id = static_cast<uint32_t>(id);

  statement.id = final_id;
  statement.type = DELETE;

  cout << "deletion parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}

Commandstatus handle_select(string sql_command, stmt &statement)
{
  stringstream ss(sql_command);
  string words;
  if (!(ss >> words))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  string leftover;
  if (ss >> leftover)
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  statement.type = SELECT;

  cout << "selection parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}

Commandstatus handle_create(string sql_command, stmt &statement, db *database)
{
  stringstream ss(sql_command);
  string first_word;
  if (!(ss >> first_word))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  string second_word;
  if (!(ss >> second_word))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  
  string table_name;
  if (!(ss >> table_name))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  
  string leftover;
  if ((ss >> leftover))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  statement.type = CREATE_TABLE;

  cout << "create table parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}

Commandstatus handle_drop(string sql_command, stmt &statement, db *database)
{
  stringstream ss(sql_command);
  string words;
  string first_word;
  if (!(ss >> first_word))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  string second_word;
  if (!(ss >> second_word))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  
  string table_name;
  if (!(ss >> table_name))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  
  string leftover;
  if ((ss >> leftover))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }

  statement.type = DROP_TABLE;

  cout << "drop table parsed." << endl;
  return Commandstatus::CMD_SUCCESS;
}

Commandstatus handle_SQL_Commands(string sql_command, stmt &statement, db *database)
{
  stringstream ss(sql_command);
  string first_word;
  if (!(ss >> first_word))
  {
    return Commandstatus::CMD_SYNTAX_ERROR;
  }
  string final_command = first_word;
  string second_word;
  if (first_word == "DROP" || first_word == "CREATE")
  {
    if (ss >> second_word && second_word == "TABLE")
    {
      final_command += " " + second_word;
    }
    else
    {
      return Commandstatus::CMD_SYNTAX_ERROR;
    }
  }

  SQLcommandType sql_command_type = getCommandType(final_command);
  Commandstatus result;

  switch (sql_command_type)
  {
  case INSERT:
    result = handle_insert(sql_command, statement, database);
    break;
  case DELETE:
    result = handle_delete(sql_command, statement);
    break;
  case UPDATE:
    result = handle_update(sql_command, statement);
    break;
  case DROP_TABLE:
    result = handle_drop(sql_command, statement, database);
    break;
  case CREATE_TABLE:
    result = handle_create(sql_command, statement, database);
    break;
  case SELECT:
    result = handle_select(sql_command, statement);
    break;
  default:
    result = Commandstatus::CMD_FAILURE;
  }

  return result;
}
